...
namespace Bludata\Doctrine\ODM\MongoDB\Traits;
    ...
    public function setPropertiesEntity(array $data)
    {
        foreach ($data as $key => $value) {
            ...
            if (method_exists($this, $methodSet) && $set) {
                ...
                /**
                 * Busca a anotação Doctrine\ODM\MongoDB\Mapping\Annotations\Field.
                 */
                    ...
                    if ($column instanceof Field && ($column->type == 'date' || $column->type == 'datetime' || $column->type == 'time')) {
                        ...
                        $ormMapping = array_filter($propertyAnnotations, function ($annotation) {
                            return $annotation instanceof ReferenceOne
                                   ||
                                   $annotation instanceof ReferenceMany
                                   ||
                                   $annotation instanceof EmbedOne
                                   ||
                                   $annotation instanceof EmbedMany;
                        });
                        ...
                        if ($ormMapping) {
                            ...
                            $targetEntityName = $reflectionClass->getNamespaceName().'\\'.$ormMapping->targetDocument;
                             ...
                             */
                            if ($ormMapping instanceof ReferenceOne) {
                                $this->$methodSet(
                                    $valueKey ? $repositoryTargetEntity->findOrCreate($valueKey) : null
                                );
                            } elseif ($ormMapping instanceof EmbedOne) {
                                $this->$methodSet(
                                    $valueKey ? $repositoryTargetEntity->createEntity($valueKey) : null
                                );
                            } elseif (($ormMapping instanceof ReferenceMany || $ormMapping instanceof EmbedMany) && is_array($valueKey)) {
                                /**
                                 ...
                                 */
                                $methodAdd = $toObject->customMethodAdd ? $toObject->customMethodAdd : 'add'.$ormMapping->targetDocument;
                                ...
                                foreach ($this->$methodGet() as $element) {
                                    ...
                                    foreach ($valueKey as $value) {
                                        if ($ormMapping instanceof ReferenceMany) {
                                            $this->$methodAdd($repositoryTargetEntity->findOrCreate($value));
                                        } elseif ($ormMapping instanceof EmbedMany) {
                                            $this->$methodAdd($repositoryTargetEntity->createEntity($value));
                                        }
                                    }
                                }
                            }
